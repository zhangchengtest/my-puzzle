...
import QimenAnalysis from './components/QimenAnalysis.vue';
...
export default {
  ...
  methods: {
    ...
    isBadPattern(cell) {
      if (!cell.tianganTianPan || !cell.tianganDiPan) return false;
      const jixiongGe = this.getJixiongGe(cell.tianganTianPan, cell.tianganDiPan);
      return jixiongGe && jixiongGe.jixiong === '凶';
    },
    // 判断是否为三奇
    isSanQi(tiangan) {
    ...
    // 获取宫位的五行
    getGongWuxingByPosition(position) {
      return this.getGongWuxing(position);
    },
    // 获取天干所在宫位
